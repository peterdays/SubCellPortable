...
def model_load(config, temp_path_suffix=None):
    ...
    if temp_path_suffix is not None:
        # ugly workaround: guarantees that triggering +1 downloads per GPU doesn't break
        encoder_path = (
            encoder_path.rsplit(".", 1)[0]
            + temp_path_suffix
            + "."
            + encoder_path.rsplit(".", 1)[1]
        )
        classifier_paths = [
            p.rsplit(".", 1)[0] + temp_path_suffix + "." + p.rsplit(".", 1)[1]
    ...
    if needs_update:
        ...
        with open(os.path.join(base_path, "models_urls.yaml"), "r") as urls_file:
            ...
            for index, curr_url_info in enumerate(
                url_info[config["model_channels"]][config["model_type"]]["classifiers"]
            ):
                if curr_url_info.startswith("s3://"):
                    try:
                        ...
                        s3.download_file(
                            urlcomponents.netloc,
                            urlcomponents.path[1:],
                            classifier_paths[index],
                        )
                        config["log"].info(
                            "  - " + classifier_paths[index] + " updated."
                        )
            ...
            curr_url_info = url_info[config["model_channels"]][config["model_type"]][
                "encoder"
            ]
            if curr_url_info.startswith("s3://"):
                try:
                    ...
                    urlcomponents = urlparse(curr_url_info)
                    s3.download_file(
                        urlcomponents.netloc, urlcomponents.path[1:], encoder_path
                    )
                ...
                if response.status_code == 200:
                    ...
                    config["log"].warning(
                        "  - " + encoder_path + " url " + curr_url_info + " not found."
                    )
